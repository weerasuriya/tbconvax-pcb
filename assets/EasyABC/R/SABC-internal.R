...
SABC.inf <- function(f.dist, d.prior, r.prior, n.sample, eps.init, iter.max,
  ...
  L <- function(eps.1, eps.2) {
    ...
    if (new.sample) {
      tmp <- k * exp(log.weights.ten + V.pri.ten) * chi.ten
    } else {
      tmp <- k * exp(V.pri.ten) * chi.ten
    }
  ...
  if (class(try(solve(Cov.uv), TRUE)) == "try-error") {
    ...
    res <- SABC.noninf(
      f.dist, d.prior, r.prior, n.sample, eps.init,
  ...
  while (kk < (iter.max - iter)) {
    ...
    if (!is.null(verbose) && kk %% verbose == 0) {
      cat(
        "ensemble updates", (kk + iter) / n.sample, "\t",
        # 'eps.1.e '  , eps.1.e, '\t',
        # 'eps.2.e'   , eps.2.e, '\t',
        "eps.1", eps.1, "\t",
        "eps.2", eps.2, "\n"
...
SABC.noninf <- function(f.dist, d.prior, r.prior,
  ...
  Schedule <- function(rho) {
    return(ifelse(rho < 1e-100, 0, uniroot(
      function(epsilon) epsilon^2 + v * epsilon^(3 / 2) - rho^2,
      c(0, rho)
    )$root))
  } #  This v is const*v in (32)
