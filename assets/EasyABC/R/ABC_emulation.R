...
ABC_emulation <- function(model, prior, nb_design_pts, nb_simul, prior_test = NULL, summary_stat_target = NULL,
  ...
  if (n_cluster != 1) {
    ...
    rejection1 <- .ABC_rejection(
      model, prior, prior_test, nb_design_pts, use_seed,
    ...
    rejection <- .ABC_rejection(
      model_emulator, prior, prior_test, nb_simul, use_seed,
      seed_count + nb_design_pts, verbose, progress_bar
    )
    res <- NULL
    if (is.null(summary_stat_target)) {
      res <- list(
        param = rejection$param, stats = rejection$stats, weights = rejection$weights,
      ...
      res <- list(
        param = rej$unadj.values, stats = rej$ss, weights = array(
          1 / nr,
          nr
        ), stats_normalization = rejection$stats_normalization, nsim = rejection$nsim,
