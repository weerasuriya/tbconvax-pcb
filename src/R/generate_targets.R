...
IN_burden <- TB_burden[country == "India", .(Year = year,
...
IN_agewise[, AgeGrp := fcase(age_group == "0-14", "[0,15)",
                             age_group == "15plus", "[15,99]",
                             age_group == "all", "[0,99]",
                             age_group == "65plus", "[65,99]")][, age_group := NULL]

MIN_agewise <- melt(IN_agewise, id.vars = c("Year", "AgeGrp"))

# Population
IN_raw_pop <- rbind(fread("data/raw_data/WPP2019_PopulationBySingleAgeSex_1950-2019.csv"),
                fread("data/raw_data/WPP2019_PopulationBySingleAgeSex_2020-2100.csv"))[Location == "India", .(Year = Time,
                                                                                                              AgeGrp,
                                                                                                              Pop = PopTotal)]
...
IN_inc_agewise <- merge(IN_agewise, IN_pop[Year==2019])[, .(Year,
                                                            ...
                                                            hi = hi/(PopTotal*1e3)*1e5)][, Indicator := "IncidenceRate"]
