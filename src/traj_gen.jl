...
@inbounds for PS in ProgressBar(axes(hm_fp, 1))
    ...
    # Update mortality
    adj_mort = CSV.File(joinpath("output", CMA, "params", "adj_mort", JN, "Adj_mort_$PSID.csv"), header = false) |> Tables.matrix |> permutedims
    ufi = merge(fi, (; demog_mort = adj_mort))

    # Vaccine simulations
    for vxt = 3, effI = [0.0], effD = [0.5], dur = 10, ageG = [[1], [2], [3], [4]], cov = [0.7]
        vx_profile = (;
            vxtype = vxt,
            effI = effI,
            effD = effD,
